...
import psycopg2
from database import get_password, add_user, get_current_user_id, add_user_words, get_one_random_word, get_three_random_words, delete_words

# Настройки Telegram-бота

try:
    # Попытка подключения к БД и проверка на ошибку
...
@bot.message_handler(commands=['cards', 'start', 's'])  # Аннотация для запуска бота
...
def create_cards(message):
    ...
    current_user_id = get_current_user_id(cid)
    ...
    if not current_user_id:
        ...
        add_user(message.from_user.first_name, message.from_user.last_name, message.from_user.username, cid)
...
def process_user_deleteinput(message):
    ...
    current_uid = get_current_user_id(cid)
...
def add_word(message):
    ...
    userStep[cid] = 1
    # Отправка сообщения в текущий чат
    bot.send_message(cid, "Введите слово на английском и русском языке ЧЕРЕЗ ПРОБЕЛ, для добавления в таблицу слов:")
...
def process_user_input(message):
    ...
    if add_user_words(eng_word_to_add, rus_word_to_add, current_uid):
    ...
    # Если при выполнении функции add_user_words вернулось False
        ...
        create_cards(message)  # Возврат на стартовую страницу в Telegram-боте
    print(add_user_words(eng_word_to_add, rus_word_to_add, current_uid))

# Функция, отвечающая за ответы в Telegram-боте
@bot.message_handler(func=lambda message: True, content_types=['text'])
def message_reply(message):
    # Присваиваем переменной text значение text из сообщения message
    text = message.text
    # Задаем количество кнопок в одной строке
    ...
    with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
        ...
        if text == target_word:  # Проверка правильности выбранного ответа
            # Присваиваем переменной hint значение, полученное в результате выполнения функции show_target
    ...
    # Отправка сообщения с ответом пользователю о правильном/неправильном переводе слова
